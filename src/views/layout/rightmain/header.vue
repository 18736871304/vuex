<template>
  ...
  >
    ...
    <div float="right">
      <!--通知按钮-->
      <el-popover placement="top-start" title="通知" width="200" trigger="click">
        ...
        <div>
          ...
          <p>
            <i class="el-icon-warning" wColor></i>这是第一条警告通知
          </p>
        </div>
        <el-divider></el-divider>
      ...
      <el-tooltip class="item" effect="dark" content="全局设置页面" placement="bottom">
        <p multiBtn rotate @click="openGlobalSetting">
...
export default {
  ...
  computed: {
    ...mapGetters({
      getCollapseState: "CollapseState", //状态改变
      getHeaderBColor: "HeaderBColor", //头部背景颜色
      getHeaderColor: "HeaderColor" //头部字体颜色
    })
  },
  methods: {
    isOpen() {
      ...
      if (this.getCollapseState == false) {
        this.$store.commit("setCollapseState", true);
      } else {
        this.$store.commit("setCollapseState", false);
      }
    },
    MQBox() {},
    //个人信息路由跳转
